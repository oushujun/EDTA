...
def BlastRef(file):
    ...
    for refLib in ref_list:
       ...
       #     file , path+"/RefLib/"+refLib, targetDir+file+spliter+"blast"+spliter+refLib)
        blast = "blastn -query %s -db %s -num_threads %s -outfmt '7 qacc sacc length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s" % (file , refLib, int(t), targetDir+file+spliter+"blast"+spliter+refLib) #shujun
        os.system(blast)
