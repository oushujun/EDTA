...
def split_by_seq_num(fasta_file, split_seq_num):
...
def split_by_seq_len(fasta_file, split_seq_len):
    records = list(SeqIO.parse(fasta_file, "fasta"))
    base_filename = os.path.splitext(fasta_file)[0]
...
def split_by_file_num(fasta_file, split_file_num):
    records = list(SeqIO.parse(fasta_file, "fasta"))
...
def main(fasta_file=None, split_seq_num=None, split_seq_len=None, split_file_num=None):
    ...
    if split_seq_num:
        split_by_seq_num(fasta_file, split_seq_num)
    elif split_seq_len:
        split_by_seq_len(fasta_file, split_seq_len)
    elif split_file_num:
        split_by_file_num(fasta_file, split_file_num)
    else:
