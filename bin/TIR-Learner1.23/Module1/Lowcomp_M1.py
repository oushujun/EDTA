...
def getLTR(rec):
    ...
    blast = "blastn -query %s -subject %s -outfmt '7 qacc sacc length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s 2>/dev/null" % ("seq1_" + ID + ".fasta", "seq2_" + ID + ".fasta", ID + "_blast.tsv") #shujun
    os.system(blast)
...
f=open("%sModule1%sLow"%(genome_Name+spliter,spliter),"w")
f.write("#Skip")
