...
def GetFastaFromFile(argsList):
    ...
    if (p1 < p2):
        ...
        p_start = p2-200
        p_end = p1+200
    seq1 = subprocess.check_output("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s' -outfmt %s" % (genomeFile+spliter+"db", entry, int(p_start), int(p_end),"%s"), shell=True)
...
def getLTR(rec):
    ...
    blast = "blastn -query %s -subject %s -outfmt '7 qseqid sseqid length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s 2>/dev/null" % ("seq1_" + ID + ".fasta", "seq2_" + ID + ".fasta", ID + "_blast.tsv") #shujun
    os.system(blast)
