...
def GetFastaFromFile(argsList):
    ...
    if (p1 < p2):
        ...
        p_start = p2-200
        p_end = p1+200
    seq1 = subprocess.check_output("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s' 2>/dev/null" % (genomeFile+spliter+"db", entry, int(p_start), int(p_end)), shell=True)
...
def getLTR(rec):
    ...
    blast = "blastn -query %s -subject %s -outfmt '7 qseqid sseqid length pident gaps mismatch qstart qend sstart send evalue qcovhsp' -out %s" % ("seq1_" + ID + ".fasta", "seq2_" + ID + ".fasta", ID + "_blast.tsv") #shujun
    os.system(blast)
#    rm = "rm %s" % ("seq2_" + ID + ".fasta")
 #   os.system(rm)
  #  rm = "rm %s 2>/dev/null" % ("seq1_" + ID + ".fasta")
   # os.system(rm)
...
#    pool.join()
#
f=open("%sModule3%sLow"%(genome_Name+spliter,spliter),"w")
f.write("#Skip")
#touch = "touch %sModule3%sLow"%(genome_Name+spliter,spliter)


#cat = "cat *.tsv > %sModule3%sLow"%(genome_Name+spliter,spliter)
#os.system(cat)
#rm = "rm *.tsv"
#rm = "rm *.tsv seq1_* seq2_* *predi*DT*.fa 2>/dev/null" #shujun
#os.system(rm)
#rm ="rm *predi*DT*.fa"
#os.system(rm)
