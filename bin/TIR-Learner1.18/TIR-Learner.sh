...
cd $dir"/Module1"
python3 $path/Module1/Fullcov.py  -name $genomeName -p $path -d $dir"/Module1" -s $species > /dev/null 2>&1
...
if [ ! -s test.select.csv ]; then
	...
	You may have specified the wrong species. Please double check and rerun TIR-Learner
	"
	rm test.select.csv
	exit
fi
...
# use TIRvish as TIR search engine
#echo "Module 2, Step 1: Split Genome and  Run GRF program to find Inverted Repeats"
