...
def GetFastaFromFile(argList):
   ...
   # seq1 = subprocess.Popen("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s' -outfmt %s" % (genomedb, entry, int(p_start), int(p_end),"%s"), shell=True)
   # seq1 = subprocess.call("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s' -outfmt %s" % (genomedb, entry, int(p_start), int(p_end),"%s"), shell=True)
   # seq1 = subprocess.call("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s'" % (genomedb, entry, int(p_start), int(p_end)), shell=True)
#    get_seq = "perl ~/las/git_bin/EDTA/util/call_seq_by_list.pl <(echo 'TIR 2Cen_mScf_10:1..1333|LTR/Gypsy:5..200') -C dmel-all-chromosome-r6.28.fasta.rename.EDTA.fa -header 0"
    get_seq = "perl ~/las/git_bin/EDTA/util/call_seq_by_list.pl <(echo 'TIR %s:%s..%s) -C %s -header 0" % (entry, int(p_start), int(p_end), genomedb)
    print(get_seq)
    out_seq1 = subprocess.Popen(['/bin/bash', '-c', get_seq])
    print(out_seq1)
 #   seq1 = seq1.decode("utf-8")
  #  out_seq1 = ''
   # split = seq1.split('\n')
    ...
    o.write(">"+genome_Name+spliter+entry+spliter+str(p1)+spliter+str(p2)+spliter+str(p_start)+spliter+str(p_end)+"_"+fam+"_200" + "\n" + str(out_seq1) + "\n")
