...
def SplitFasta(contigFile, genomeFile, genomeName):
    ...
    with open(contigFile) as f: # Sky fix 281 20230419
        l_name = [line.strip() for line in f.readlines()]
    for name in l_name:
        SeqIO.write((rec for rec in records if rec.id.split(":")[0] == name),
                    "%s.fasta" % (targetDir + genomeName + spliter + name), "fasta")


os.chdir(tempDir)
files = os.listdir(".")
p_files = [i for i in files if
           i.split(spliter)[0] == genome_Name and i.split(spliter)[-1] == "p" and i.split(spliter)[-2] == "GRFmite.fa"]
    ...
    for file in files:
        ...
        if (len(str(records[0].seq)) > int(length) + 500):
            grf = "%s/grf-main -i %s -o %s -c 1 -t %s -p 20 --min_space 10 " \
                  "--max_space %s --max_indel 0 --min_tr 10 --min_spacer_len 10 --max_spacer_len %s" % (
                  grfP, file, file.split(spliter)[1][:-6], int(t), int(length), int(length))
            os.system(grf)
