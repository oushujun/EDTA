...
parser.add_argument("-s", "--species", help="One of the following: Maize , Rice or others", required=True, default="Maize")
...
#       for line in preNamelist:
    ...
    if (p1 < p2):
        ...
        if (p2 > 200):
            ...
            p_start = 1
        p_end = p1+200
    seq1 = subprocess.check_output("blastdbcmd -db '%s' -entry '%s' -range '%s'-'%s' 2>/dev/null" % (genomedb, entry, int(p_start), int(p_end)), shell=True)
...
if __name__ == '__main__':
    ...
    files=os.listdir(".")
    fastafile=[i for i in files if i[-1]=="p"]
    prefiles=[j+spliter+"toPre.csv"+spliter+"predi.csv" for j in fastafile]
