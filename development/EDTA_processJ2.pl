...
`mkdir $genome.EDTA.combineJ2` unless -e "$genome.EDTA.combineJ2" && -d "$genome.EDTA.combineJ2";

# enter the combine folder for EDTA processing
#chdir "$genome.EDTA.combine";
chdir "$genome.EDTA.combineJ2";
...
## prefilter TIRs with LTR regions and HelitronScanner
# extract LTR regions as HQ
`grep \"_LTR\" $LTR > $LTR.list`;
`perl $output_by_list 1 $LTR 1 $LTR.list -FA > $LTR.LTR`;

# identify Helitron contaminants with HelitronScanner
`sh $HelitronScanner $TIR $threads`;
`cat $TIR.HelitronScanner.draw.hel.fa $TIR.HelitronScanner.draw.rc.hel.fa | perl -nle \'print \$_ and next unless /^>/; my \$line=(split)[0]; \$line=~s/\#SUB_//; \$line=~s/[\#\\/]/_/g; print \"\$line\#DNA\/Helitron\"\' > $TIR.helitron`;
`cat $script_path/database/HelitronScanner.training.set.fa >> $TIR.helitron`;

# remove potential LTR and helitron contaminants
`cat $LTR.LTR $TIR.helitron > $LTR.LTR_$TIR.helitron`;
`${repeatmasker}RepeatMasker -pa $threads -q -no_is -norna -nolow -div 40 -lib $LTR.LTR_$TIR.helitron $TIR 2>/dev/null`;
`perl $cleanup_tandem -misschar N -nc 50000 -nr 0.9 -minlen 80 -minscore 3000 -trf 0 -cleanN 1 -cleanT 1 -f $TIR.masked > $TIR.HQ`;

## prefilter Helitrons with LTR regions
`${repeatmasker}RepeatMasker -pa $threads -q -no_is -norna -nolow -div 40 -lib $LTR.LTR $HEL 2>/dev/null`;
`perl $cleanup_tandem -misschar N -nc 50000 -nr 0.9 -minlen 100 -minscore 3000 -trf 0 -cleanN 1 -cleanT 1 -f $HEL.masked > $HEL.pre`;
...
if (0){
# purify LTR
&Purifier("$LTR", "$TIR.HQ", $mindiff_LTR);
&Purifier("$LTR.HQ", "$HEL", $mindiff_LTR);
`mv $LTR.HQ.HQ $LTR.HQ`;
}

# purify Helitron
&Purifier("$HEL.pre", "$TIR.HQ", $mindiff_HEL);
&Purifier("$HEL.pre.HQ", "$LTR", $mindiff_HEL);
`mv $HEL.pre.HQ.HQ $HEL.HQ`;
...
`cat $TIR.HQ $HEL.HQ | perl -nle 's/>/\\n>/g unless /^>/; print \$_' > $genome.TIR.Helitron.fa.stg1.raw.cln`;
...
`perl $rename_TE $genome.LTR.TIR.Helitron.fa.stg1.raw.cln > $genome.LTR.TIR.Helitron.fa.stg1`; #quick line for testing
